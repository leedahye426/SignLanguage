...
def correctPose(current_angles1, word):
    ...
    if (current_angles[0] < (correct_angles[0] * 0.4)) or (current_angles[0] > (correct_angles[0] * 1.6)) :
        tmp = "X"
        return tmp
    elif (current_angles[1] < (correct_angles[1] * 0.4)) or (current_angles[1] > (correct_angles[1] * 1.6)):
        tmp = "X"
        return tmp
    elif (current_angles[2] < (correct_angles[2] * 0.4)) or (current_angles[2] > (correct_angles[2] * 1.6)):
        tmp = "X"
        return tmp
    elif (current_angles[3] < (correct_angles[3] * 0.4)) or (current_angles[3] > (correct_angles[3] * 1.6)):
        tmp = "X"
        return tmp
