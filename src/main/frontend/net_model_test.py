...
while cap.isOpened():
    ...
    if result.multi_hand_landmarks is not None:
        ...
        if results.pose_landmarks is not None:
            ...
            end_time = time.time()
            ##cv2.putText(img, f'Waiting for collecting NOODLES action...', org=(10, 30), fontFace=cv2.FONT_HERSHEY_SIMPLEX, fontScale=1, color=(255, 255, 255), thickness=2)
            cv2.putText(img, f'{this_action.upper()}', org=(int(res.landmark[0].x * img.shape[1]), int(res.landmark[0].y * img.shape[0] + 20)), fontFace=cv2.FONT_HERSHEY_SIMPLEX, fontScale=1, color=(255, 255, 0), thickness=2)
